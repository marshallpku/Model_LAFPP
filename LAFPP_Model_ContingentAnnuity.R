...
get_contingentAnnuity <- function(Tier_select,
  ...
  factor.ca <- tier.param[Tier_select, "factor.ca"]
  cola      <- tier.param[Tier_select, "cola"]
...
get_liab.ben <- function(gender.R, 
  ...
  mortality <- 
       ...
       # %>% This part is unnecessary for LAFPP
...
# w/ benefit reduction
df.M <- get_liab.ben(gender.R = "M", reduction = ifelse(apply_reduction, reduction.M, 1))

# w/ benefit reduction
df.F <- get_liab.ben(gender.R = "F", reduction = ifelse(apply_reduction, reduction.F, 1))
